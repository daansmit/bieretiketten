  ...
  return (
    <>
      <div className="side-panel-header">
        ...
        <button onClick={onClose} title="Sluiten">✕</button>
      </div>
      ...
      <div className="side-panel-content">
        ...
        <div className="row-info-card">
          ...
          <div className="row-detail">
            {[row.brouwerij, row.plaatsnaam, row.land].filter(Boolean).join(' · ')}
          </div>
          {row.alcohol !== '' && (
            <div className="row-detail">🍺 {row.alcohol}% · {row.soort}</div>
          )}
        </div>
        ...
        {pdfFiles.map((file) => (
          <div className="pdf-item" key={file}>
            ...
            <div className="pdf-info">
              <div className="pdf-name" title={basename(file)}>{basename(file)}</div>
            </div>
            <button
